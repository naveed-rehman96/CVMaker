...
class PreviewDarkRedWhiteActivity : AppCompatActivity() {
    ...
    private fun getOPFile(): File? {
        ...
        return File(mediaStorageDir.path + File.separator + modelMain.personInfo.fileName + "_DRWRED.jpg")
    }
    ...
    fun previewCV() {
        ...
        if (!tinyDB.getBoolean(Constants.skipLang)) {
            htmlContent.append(
                String.format(
                    " <h1 class=\"c9\" style=\" margin-bottom: 15px;\">" +
                            "<span class=\"c16\" style=\" margin-bottom: 2px;\">Languages</span>\n" +
                            "                        </h1>\n" +
                            "\n"
                )
            )
